...
*/
		...
		JLabel lblNewLabel_4 = new JLabel("Borrar el historial de: ");
	...
	//	panelModificar.add(pSurE, BorderLayout.SOUTH);
		...
		btnBuscar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				cambio = false;
				String historial;
				ArrayList<Historial> aHistoriales = null;
		...
		modelotabla.addTableModelListener(new TableModelListener() {
			...
			public void tableChanged(TableModelEvent e) {
				if(cambio) {
					...
					ArrayList<Historial> aHistoriales = BD.obtenerListaHistorial(con);
					aHistoriales.get(fila).setDesc(desc);
					
					BD.modificarHistorial(con, aHistoriales.get(fila).getDesc(), aHistoriales.get(fila).getDni());
	...
	private void insertarHistorial() {
		...
		if (!(textFieldDesc.getText().isEmpty())) {
			...
			if (nombrePaciente != null) {
				...
				if(BD.buscarHistorial(con, h.getDni()) == null)
					BD.anadirHistorial(con, h);
				else
					JOptionPane.showMessageDialog(null, "DNI ya existente", "ERROR", JOptionPane.ERROR_MESSAGE);
			} else {
